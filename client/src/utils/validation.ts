...
export const validationInputs=(
    ...
    const addDangerBorder=()=>{
        inputs.forEach(input=>input.value.length?input.classList.remove('border-danger'):
        input.classList.add('border-danger'))
    }
    if(!textInputValue || !dateInputValue||!priceInputValue ){
        handleAlertMessage({
            alertText:'Заполните все поля',alertStatus:'warning'
    ...
    if (isNaN(+priceInputValue)){
        handleAlertMessage({
            alertText:'Введите число',alertStatus:'warning'
        })
        addDangerBorder()
